...
\section{Simulation of the CIR process}
  ...
  For the CIR process (\ref{CIR}), when the following inequality is satisfied,
  \begin{equation}
    2\kappa\theta >\sigma^2,
  \end{equation}
  the process will never reach the $v=0$ boundary. This is known as the {\it Feller condition}. When the Feller
  condition is violated, {\it i.e.}, $2\kappa\theta \le\sigma^2$, the process will hit the $v=0$ boundary, and a
  proper boundary condition is required. To preserve the total probability, a natural choice is the reflecting
  boundary condition, and once the process hit the lower boundary, it will be completely reflected. Therefore,
  for meaningful CIR parameters, we can always impose the reflecting boundary condition, and the
  probability distribution of $v_{t+\Delta}$ conditional on $v_t$ is given by (\ref{reflecting}),
  \begin{equation}
    p(\left.v_{t+\Delta}\right|v_t)
  \end{equation}
